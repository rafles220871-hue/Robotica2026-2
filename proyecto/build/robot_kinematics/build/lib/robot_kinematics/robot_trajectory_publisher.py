...
class PublicadorTrayectoria(Node):
  ...
  # Callback de posición deseada como PointStamped (CLIC EN RVIZ)
  def ps_callback(self, msg:PointStamped):
    ...
    # 1. Encontrar los índices dinámicamente sin importar cómo los ordene ROS
    idx_shoulder = self.js_current.name.index("shoulder_joint")
    idx_arm      = self.js_current.name.index("arm_joint")
    idx_forearm  = self.js_current.name.index("forearm_joint")

    # 2. Asignarlos correctamente a th_i
    self.robot.def_tray(
      th_i=(self.js_current.position[idx_shoulder],
            self.js_current.position[idx_arm],
            self.js_current.position[idx_forearm]), 
      xi_f=(msg.point.x, 
            msg.point.y, 
            msg.point.z)) 
