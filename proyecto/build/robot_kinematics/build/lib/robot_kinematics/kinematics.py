...
import numpy as np # IMPORTANTE: Agregamos numpy para el cálculo numérico

class Robot():
  def __init__(self, l:tuple[float]=(0.3, 0.3, 0.3)):
    th1, th2, th3 = symbols("theta_1,theta_2,theta_3")

    # 1. Transformaciones 3D que coinciden exactamente con tu URDF antropomórfico
    T_0_1 = self.tr_h(alpha=th1)
    T_1_2 = self.tr_h(z=l[0]) * self.tr_h(gamma=1.5708) * self.tr_h(alpha=th2)
    T_2_3 = self.tr_h(x=l[1]) * self.tr_h(alpha=th3)
  ...
  def def_tray(self, t_f:float=2, frec:float=15, 
    ...
    # Importante: evaluar a float desde el inicio
